...
export default function Header() {
  ...
  const openPanel = (id: DropID) => {
    if (closeTimer.current) clearTimeout(closeTimer.current);
    setActivePanel(id);
  };
  const scheduleClose = () => {
    closeTimer.current = setTimeout(() => setActivePanel(null), 140);
  };
  const closeAll = () => setActivePanel(null);
  const goCategory = (cat: string) => {
    closeAll();
    navigate(`/marketplace?category=${cat}`);
  };
  const goSell = () => {
    closeAll();
    navigate("/marketplace?sell=1");
  };
            ...
            {[
              {
                cat: "electronics",
                label: "Electronics",
                sub: "MacBooks, iPhones",
              },
              { cat: "fashion", label: "Fashion", sub: "Nike, Uniqlo" },
              {
                cat: "jewellery",
                label: "Jewellery",
                sub: "Watches, rings & chains",
              },
      ...
      return (
        <div className="header-dropdown-content p-4 w-52">
          ...
          <p className="text-xs text-gray-500 leading-relaxed mb-3">
            Find and compare products — prices, ratings and sellers in one
            place.
          </p>
          <Link
            to="/search"
            onClick={closeAll}
            className="flex items-center justify-center w-full py-2 rounded-lg text-xs font-semibold text-white transition-all duration-150 hover:opacity-90"
    ...
    if (id === "wishlist")
      return (
        <div className="header-dropdown-content p-4 w-56">
          ...
          <p className="text-xs text-gray-500 leading-relaxed mb-3">
            Set target prices — we track live prices and flag drops.
          </p>
          <div className="flex flex-col gap-1.5">
            <Link
              to="/wish-list"
              onClick={closeAll}
              className="flex items-center justify-center py-2 rounded-lg text-xs font-semibold text-white transition-all duration-150 hover:opacity-90"
              style={{ background: GRAD }}
            >
              My Wish List
            </Link>
    ...
    if (id === "about")
      return (
        <div className="header-dropdown-content p-4 w-56">
          ...
          <p className="text-xs text-gray-500 leading-relaxed mb-3">
            We verify listings so you shop with confidence — no fakes, no price
            inflation.
          </p>
          <Link
            to="/what-is-verifind"
  ...
  const navLinks = [
    ...
    ...(loggedIn
      ? [{ to: "/messages", label: "Messages", dropID: null as DropID }]
      : []),
    {
      to: "/what-is-verifind",
      label: "What is Verifind?",
      dropID: "about" as DropID,
    },
  ];
  ...
  return (
    <>
      {/* Hover zone */}
      <div
        className="fixed top-0 left-0 right-0 z-40 h-3"
