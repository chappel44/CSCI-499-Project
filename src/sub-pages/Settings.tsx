...
export default function Settings() {
  ...
  const [blockedUsersMessage, setBlockedUsersMessage] = useState<string | null>(null);
  const [unblockingId, setUnblockingId] = useState<string | null>(null);

  const fileInputRef = useRef<HTMLInputElement | null>(null);
  const blockedUsersRef = useRef<HTMLDivElement | null>(null);
  ...
  return (
    <div className="profile-page min-h-screen bg-gray-100 flex items-center justify-center px-4 py-10">
      <div className="w-full max-w-5xl flex flex-col gap-5">
            ...
            <div className="px-8 py-8 flex flex-col gap-6">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() => blockedUsersRef.current?.scrollIntoView({ behavior: "smooth", block: "start" })}
                    className="profile-secondary-button rounded-xl border border-gray-200 px-4 py-2 text-sm font-semibold text-gray-700 transition hover:bg-gray-50"
                  >
                    Blocked Users
                  </button>
                </div>
        ...
        <div ref={blockedUsersRef} className="profile-card bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
          <div className="px-8 py-8 flex flex-col">
            <h2 className="text-xl font-bold text-gray-900 mb-1">Blocked Users</h2>
            <p className="text-sm text-gray-400 mb-5">
              Manage sellers you blocked from Marketplace. You can unblock anytime.
            </p>
                  ...
                  >
                    <div>
                      <p className="blocked-user-name text-sm font-semibold text-gray-900">
                        {blockedUser.username ? `@${blockedUser.username}` : `User ${blockedUser.blocked_id.slice(0, 8)}`}
                      </p>
                      <p className="blocked-user-meta text-xs text-gray-500 mt-1">
                        Blocked on {new Date(blockedUser.created_at).toLocaleDateString()}
                      </p>
                      {blockedUser.reason && (
                        <p className="blocked-user-meta text-xs text-gray-500 mt-1">Reason: {blockedUser.reason}</p>
                      )}
                    </div>
                    <button
                      type="button"
